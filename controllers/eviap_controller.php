...
try {
    // CONSULTA PARA OBTENER LAS EVIDENCIAS REALIZADAS EN ZAJUNA
    $titulos = $conn->prepare("SELECT obtenerEvidenciasAp(:curso, :rea)");
    $titulos->bindParam(':curso', $id_curso, PDO::PARAM_STR);
    $titulos->bindParam(':rea', $id_rea, PDO::PARAM_INT);
    $titulos->execute();
    $evi_query = "SELECT * FROM vista_evidenciasAp";
    $titulos = $conn->prepare($evi_query);
    $titulos->execute();
